...
if [[ "${EXECUTERM_SKIP_ZIG_BUILD:-${CMUX_SKIP_ZIG_BUILD:-}}" == "1" ]]; then
  echo "Skipping zig CLI helper build (EXECUTERM_SKIP_ZIG_BUILD=1)"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/executerm-ghostty-helper.XXXXXX")"
